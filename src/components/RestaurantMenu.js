...
const RestaurantMenu = () => {
  ...
  console.log("resinfo=>",restInfo?.cards[2]?.card?.card?.info?.costForTwoMessage)
  ...
  return (
    <div className="text-center">
      <h1 className="font-bold my-8 text-2xl">{restInfo?.cards[2]?.card?.card?.info?.name}</h1>
      <p className="font-bold text-lg">{restInfo?.cards[2]?.card?.card?.info?.cuisines.join(",")} - {restInfo?.cards[2]?.card?.card?.info?.costForTwo/100}</p>
      {/* <ul>
        {
          itemCards.map(r => <li>{r.card.info.name}</li>)
        }
      </ul> */}
      {/* categories accordian */}
      {
        categories.map((category,i) => <RestaurantCategory key={category?.card?.card?.title} data = {category?.card?.card}/>)
      }
    </div>
  )
}
